		...
		if( $keyword !== NULL )
		{
			...
			if( $recurringData['repeat_end_occurrences'] )
			{
				...
				foreach( $period as $dateOccurrence )
				{
					...
					{
						$results[]	= array( 'startDate' => new \IPS\calendar\Date( $dateOccurrence->mysqlDatetime( FALSE, TRUE ) ), 'endDate' => $eDate );
					}
	...
	public static function formElements( $item=NULL, \IPS\Node\Model $container=NULL )
	{
		...
		$return['dates']		= new \IPS\Helpers\Form\Custom( 'event_dates', $dateValues, FALSE, array( 'getHtml' => function( $element )
		{
		...
		} ), function( $val )
			{
				...
				if( isset( $val['no_end_time'] ) AND $val['no_end_time'] AND isset( $val['end_time'] ) )
				{
					unset( $val['end_time'] );
				}
