			...
			foreach( $this->_parsedIcsData['events'] as $event )
			{
				...
				elseif( isset( $event['location'] ) )
				{
					try
					{
						...
						/* We get by LatLong because getLatLong() does not populate the other address data. */
						$newEvent->location	= json_encode( \IPS\GeoLocation::getByLatLong( $location->lat, $location->long ) );
					}
					catch( \BadFunctionCallException $e ){}
				}
