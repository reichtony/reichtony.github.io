...
class _Server extends \IPS\Node\Model
{
	...
	{
		/* Initiate an object */
		$classname = 'IPS\\nexus\Hosting\\' . mb_ucfirst( $data['server_type'] ) . '\\Server';
		$obj = new $classname;
		$obj->_new = FALSE;
	...
	public function form( &$form )
	{
		...
		foreach ( new \DirectoryIterator( \IPS\ROOT_PATH . '/applications/nexus/sources/Hosting' ) as $file )
		{
			if ( !$file->isDot() and mb_substr( $file, 0, 1 ) !== '.' and $file->isDir() )
			{
				...
				$class = 'IPS\nexus\Hosting\\' . mb_ucfirst( $file ) . '\Server';
	...
	public function formatFormValues( $values )
	{
		...
		$values = call_user_func( array( 'IPS\nexus\Hosting\\' . mb_ucfirst( $values['server_type'] ) . '\Server', '_formatFormValues' ), $values );
