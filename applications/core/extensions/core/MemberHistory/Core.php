					...
					case 'purchase':
						...
						switch ( $jsonValue['action'] )
						{
							...
							case 'change':
								...
								{
									$expiringPurchase = $val['remove_name'];
								}
				...
				{			
					...
					try
					{
						...
						if ( $warning->canViewDetails() )
						{
							...
							{
								return \IPS\Member::loggedIn()->language()->addToStack('history_received_warning_link', FALSE, array( 'sprintf' => array( \IPS\Http\Url::internal("app=core&module=members&controllers=members&do=viewWarning&id={$warning->id}"), $jsonValue['points'], \IPS\Member::loggedIn()->language()->addToStack( 'core_warn_reason_' . $jsonValue['reason'] ), $byStaff, $consequences ) ) );
							}
							else
							{
								return \IPS\Member::loggedIn()->language()->addToStack('history_received_warning_legacy_link', FALSE, array( 'sprintf' => array( \IPS\Http\Url::internal("app=core&module=members&controllers=members&do=viewWarning&id={$warning->id}") ) ) );
							}
						}
					}
					catch ( \Exception $e ) { }
					...
					if ( isset( $jsonValue['points'] ) )
					{
						return \IPS\Member::loggedIn()->language()->addToStack('history_received_warning_details', FALSE, array( 'sprintf' => array( $jsonValue['points'], \IPS\Member::loggedIn()->language()->addToStack( 'core_warn_reason_' . $jsonValue['reason'] ), $byStaff, $consequences ) ) );
					}
					else
					{
						return \IPS\Member::loggedIn()->language()->addToStack( 'history_received_warning', FALSE, array( 'sprintf' => array( $byStaff ) ) );
					}
				}
				break;
