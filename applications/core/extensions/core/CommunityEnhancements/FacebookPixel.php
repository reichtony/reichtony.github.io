...
class _FacebookPixel
{
	...
	public function toggle( $enabled )
	{
		if ( $enabled )
		{
			...
			{
				\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => 1 ), array( 'conf_key=?', 'fb_pixel_enabled' ) );
		...
		{
			\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => 0 ), array( 'conf_key=?', 'fb_pixel_enabled' ) );
			unset( \IPS\Data\Store::i()->settings );
		}
	}
}