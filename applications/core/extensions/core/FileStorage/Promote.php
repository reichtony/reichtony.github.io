...
class _Promote
{
	...
	public function move( $offset, $storageConfiguration, $oldConfiguration=NULL )
	{
		...
		foreach( json_decode( $data['promote_media'], TRUE ) as $location )
		{
			if ( $location )
			{
				try
				{
					\IPS\File::get( $oldConfiguration ?: 'core_Promote', $location )->move( $storageConfiguration );
				}
				catch( \Exception $e )
				{
	...
	{
		$data = \IPS\Db::i()->select( '*', 'core_social_promote', array( "promote_media!=?", '[]' ), 'promote_id', array( 0, 1 ) )->first();
		$fixed = array();
