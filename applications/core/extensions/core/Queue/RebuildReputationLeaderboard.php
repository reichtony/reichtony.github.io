...
class _RebuildReputationLeaderboard
{
	...
	public function run( &$data, $offset )
	{
		...
		for( $i = 0; $i < $this->rebuild; $i++ )
		{
			...
			foreach( \IPS\Db::i()->select( 'core_reputation_index.member_received as member, SUM(rep_rating) as rep', 'core_reputation_index', $where, 'rep DESC', 4, 'member' )->join( 'core_members', array( 'core_reputation_index.member_received = core_members.member_id' ) )->setKeyField('member')->setValueField('rep') as $member => $rep )
			{
				if ( $member and $rep )
				{
