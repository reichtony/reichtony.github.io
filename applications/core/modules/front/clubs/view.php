...
class _view extends \IPS\Helpers\CoverPhoto\Controller
{
	...
	protected function join()
	{
		...
		if ( in_array( $this->memberStatus, array( \IPS\Member\Club::STATUS_INVITED, \IPS\Member\Club::STATUS_INVITED_BYPASSING_PAYMENT, \IPS\Member\Club::STATUS_WAITING_PAYMENT ) ) or $this->club->type === \IPS\Member\Club::TYPE_OPEN or \IPS\Member::loggedIn()->modPermission('can_access_all_clubs') )
		{
			...
			{
				if ( $fee = $this->club->joiningFee() )
				{
					/* Create the item */		
					$item = new \IPS\core\extensions\nexus\Item\ClubMembership( $this->club->name, $fee );
					$item->id = $this->club->id;
					try
					{
					...
					$item->renewalTerm = $this->club->renewalTerm( $fee->currency );
					$item->payTo = $this->club->owner;
					...
					$invoice->return_uri = "app=core&module=clubs&controller=view&id={$this->club->id}";
					$invoice->save();
	...
	protected function nodeDelete()
	{
		...
		if ( !$node->modPermission( 'delete', \IPS\Member::loggedIn() ) and $itemClass::contentCount( $node, TRUE, TRUE, TRUE, 1 ) )
		{
