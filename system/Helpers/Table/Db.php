...
class _Db extends Table
{
	...
	public function getRows( $advancedSearchValues )
	{
		...
		$query = \IPS\Db::i()->select( '*', $this->table, NULL, NULL, array( 0, 1 ) );
		...
		foreach ( $select as $rowId => $row )
		{
			...
			/* Highlighting? */
			if ( isset( $this->parsers['_highlight'] ) and $class = call_user_func( $this->parsers['_highlight'], $_row ) )
			{
