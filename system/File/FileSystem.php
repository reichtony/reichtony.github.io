...
class _FileSystem extends \IPS\File
{
	...
	public function contents( $refresh=FALSE )
	{
		if ( $this->contents === NULL or $refresh === TRUE )
		{
			if( file_exists( $this->configuration['dir'] . '/' . $this->container . '/' . $this->filename ) )
			{
				...
				{
					throw new \IPS\File\Exception( $this->container . '/' . $this->filename, \IPS\File\Exception::CANNOT_OPEN );
				}
			}
			else
			{
				throw new \IPS\File\Exception( $this->container . '/' . $this->filename, \IPS\File\Exception::DOES_NOT_EXIST );
			}
		}
		...
		if( $this->temporaryFilePath )
		{
			if( static::$copyFiles === TRUE )
			{
				if( !@\copy( $this->temporaryFilePath, "{$folder}/{$this->filename}" ) )
				{
					...
					throw new \IPS\File\Exception( "{$folder}/{$this->filename}", \IPS\File\Exception::CANNOT_COPY );
				}
			}
			else
			{
				if( !@\rename( $this->temporaryFilePath, "{$folder}/{$this->filename}" ) )
				{
					...
					throw new \IPS\File\Exception( "{$folder}/{$this->filename}", \IPS\File\Exception::CANNOT_MOVE );
				}
			}
		...
		else
		{
			...
			{				
				...
				{					
					...
					throw new \IPS\File\Exception( "{$folder}/{$this->filename}", \IPS\File\Exception::CANNOT_WRITE );
				}
	...
	{		
		...
		$debug = array_filter( $immediateCaller[0], function( $key ) { return in_array( $key, array( 'class', 'function', 'line' ) ); }, ARRAY_FILTER_USE_KEY );
