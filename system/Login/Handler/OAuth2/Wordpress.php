...
class _Wordpress extends \IPS\Login\Handler\OAuth2
{
	...
	public function userProfilePhoto( \IPS\Member $member )
	{
		...
		{			
			...
			if ( preg_match( '/^https?:\/\/(?:secure\.|www\.)?gravatar\.com\/avatar\/(.{32})/', $url, $matches ) )
			{
				$url = \IPS\Http\Url::external( "https://secure.gravatar.com/avatar/" . $matches[1] )->setQueryString( 'd', '404' );
				try
				{
