...
class _LinkedIn extends \IPS\Login\Handler\OAuth2
{
	...
	protected function authenticatedUserName( $accessToken )
	{
		...
		{
			return (string) $this->_userData( $accessToken )['formattedName'];
		}
		return NULL;
	}
	...
	{
		return $this->_userData( $accessToken )['emailAddress'];
	}
	...
	public function userProfilePhoto( \IPS\Member $member )
	{
		...
		if ( isset( $userData['pictureUrls'] ) AND isset( $userData['pictureUrls']['_total'] ) AND $userData['pictureUrls']['_total'] )
		{
			foreach ( $this->_userData( $link['token_access_token'] )['pictureUrls']['values'] as $picture )
			{
				return \IPS\Http\Url::external( $picture );
			}
		}
		return NULL;
	}
	...
	public function userProfileName( \IPS\Member $member )
	{
		...
		return $this->_userData( $link['token_access_token'] )['formattedName'];
	}
	...
	{		
		return \IPS\Http\Url::external( "https://www.linkedin.com/in/" )->setQueryString( 'id', $identifier );
	}
