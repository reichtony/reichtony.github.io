...
class _Microsoft extends \IPS\Login\Handler\OAuth2
{
	...
	{
		return \IPS\Http\Url::external('https://login.live.com/oauth20_authorize.srf');
	}
	...
	{
		return \IPS\Http\Url::external('https://login.live.com/oauth20_token.srf');
	}
	...
	{
		return $this->_userData( $accessToken )['emails']['preferred'];
	}
	...
	public function userProfilePhoto( \IPS\Member $member )
	{
		...
		$url = \IPS\Http\Url::external( "https://apis.live.net/v5.0/me/picture" )->setQueryString( 'access_token', $link['token_access_token'] );
		if ( $url->request()->head()->httpResponseCode == 404 )
		{
			return NULL;
		}
		return $url;
	...
	protected function _userData( $accessToken )
	{
		...
		{
			$response = \IPS\Http\Url::external( "https://apis.live.net/v5.0/me" )
				->request()
				->setHeaders( array(
