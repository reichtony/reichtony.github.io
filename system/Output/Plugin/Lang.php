...
class _Lang
{
	...
	{		
		...
		{
			$return['pre'] .= '$sprintf = \IPS\Member::loggedIn()->language()->formatList(' . $options['list'] . ');';
			$params[]= '\'sprintf\' => $sprintf';
		}
		
		if ( isset( $options['htmllist'] ) )
		{
			$return['pre'] .= '$htmlsprintf = \IPS\Member::loggedIn()->language()->formatList(' . $options['htmllist'] . ');';
