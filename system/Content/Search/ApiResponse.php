...
class _ApiResponse extends \IPS\Api\PaginatedResponse
{
	...
	public function getOutput()
	{
		...
		$searchResults = $queryObject->search(
			$this->select[2],
			$this->select[3],
			$this->select[1] + \IPS\Content\Search\Query::TAGS_MATCH_ITEMS_ONLY,
			$this->select[4]
		);
		...
		foreach ( $searchResults as $result )
		{
			...
			$results[] = array(
				'title'			=> $resultData['itemData'][ $itemClass::$databasePrefix . $itemClass::$databaseColumnMap['title'] ],
