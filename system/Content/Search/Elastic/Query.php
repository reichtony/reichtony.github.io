...
class _Query extends \IPS\Content\Search\Query
{
	...
	public function search( $term = NULL, $tags = NULL, $method = 1, $operator = NULL )
	{
		...
		try
		{
			...
			$return = $this->url->setPath( $this->url->data[ \IPS\Http\Url::COMPONENT_PATH ] . '/_search' )->request()->setHeaders( array( 'Content-Type' => 'application/json' ) )->get( json_encode( array(
				'query'	=> $query,
