...
abstract class _Comment extends \IPS\Content
{
	 ...
	 */
	protected function checkProfanityFilters( $first=FALSE, $edit=TRUE )
	{
		/* We need our item */
		$item = $this->item();
	...
	public function editContents( $newContent )
	{
		...
		/* Resync the item - necessary as profanity filters may have just hidden an otherwise approved post  */
		$item = $this->item();
		if( $isHiddenNow != $this->hidden() )
		{
			$item->resyncCommentCounts();
